...
async function verifyMatchAccess(matchId, userId) {
  ...
  if (!match.users.includes(userId)) {
    return { error: "Access denied: You are not part of this match", status: 403 };
  }
...
export const getMessages = async (req, res) => {
  try {
    ...
    if (isBlocked) {
      return res.status(403).json({ success: false, message: "Cannot view messages - user blocked" });
    }
...
export const markMessagesSeen = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Error marking messages seen", error: error.message });
  }
};
...
export const getLastMessages = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Error fetching last messages", error: error.message });
  }
};
